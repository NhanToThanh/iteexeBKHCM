...
class Node(Persistable):
    ...
    def __init__(self, package, parent=None, title=""):
        ...
        self._id = package.regNewNode(self)
    ...
    #    if hasattr(self, 'idTarget'):
    #       return self.idTarget
    #   else:
    #       return None
    #idTarget = property(getTargetId)
    ...
    def copyToPackage(self, newPackage, newParentNode=None):
        ...
        try:
            ...
            newNode = deepcopy(self, {id(self._package): newPackage,
                                      id(self.parent): None})
            newNode._id = newPackage.regNewNode(newNode)

            # Add target ID
            newNode.idTarget = newPackage.regTargetNode(newNode)
            # Add target ID
        ...
        for node in newNode.walkDescendants():
            node._id = newPackage.regNewNode(node)
            node.idTarget = newPackage.regTargetNode(newNode)
