lomVocab = {
    "interactivityTypeValues": [
        [
            "active", 
            _("active")
        ], 
        [
            "expositive", 
            _("expositive")
        ], 
        [
            "mixed", 
            _("mixed")
        ]
    ], 
    "learningResourceTypeValues": [
        [
            "exercise", 
            _("exercise")
        ], 
        [
            "simulation", 
            _("simulation")
        ], 
        [
            "questionnaire", 
            _("questionnaire")
        ], 
        [
            "diagram", 
            _("diagram")
        ], 
        [
            "figure", 
            _("figure")
        ], 
        [
            "graph", 
            _("graph")
        ], 
        [
            "index", 
            _("index")
        ], 
        [
            "slide", 
            _("slide")
        ], 
        [
            "table", 
            _("table")
        ], 
        [
            "narrative text", 
            _("narrative text")
        ], 
        [
            "exam", 
            _("exam")
        ], 
        [
            "experiment", 
            _("experiment")
        ], 
        [
            "problem statement", 
            _("problem statement")
        ], 
        [
            "self assessment", 
            _("self assessment")
        ], 
        [
            "lecture", 
            _("lecture")
        ]
    ], 
    "aggregationLevelValues": [
        [
            "1", 
            "1"
        ], 
        [
            "2", 
            "2"
        ], 
        [
            "3", 
            "3"
        ], 
        [
            "4", 
            "4"
        ]
    ], 
    "purposeValues": [
        [
            "discipline", 
            _("discipline")
        ], 
        [
            "idea", 
            _("idea")
        ], 
        [
            "prerequisite", 
            _("prerequisite")
        ], 
        [
            "educational objective", 
            _("educational objective")
        ], 
        [
            "accessibility restrictions", 
            _("accessibility restrictions")
        ], 
        [
            "educational level", 
            _("educational level")
        ], 
        [
            "skill level", 
            _("skill level")
        ], 
        [
            "security level", 
            _("security level")
        ], 
        [
            "competency", 
            _("competency")
        ]
    ], 
    "roleMetaValues": [
        [
            "creator", 
            _("creator")
        ], 
        [
            "validator", 
            _("validator")
        ]
    ], 
    "roleValues": [
        [
            "author", 
            _("author")
        ], 
        [
            "publisher", 
            _("publisher")
        ], 
        [
            "unknown", 
            _("unknown")
        ], 
        [
            "initiator", 
            _("initiator")
        ], 
        [
            "terminator", 
            _("terminator")
        ], 
        [
            "validator", 
            _("validator")
        ], 
        [
            "editor", 
            _("editor")
        ], 
        [
            "graphical designer", 
            _("graphical designer")
        ], 
        [
            "technical implementer", 
            _("technical implementer")
        ], 
        [
            "content provider", 
            _("content provider")
        ], 
        [
            "technical validator", 
            _("technical validator")
        ], 
        [
            "educational validator", 
            _("educational validator")
        ], 
        [
            "script writer", 
            _("script writer")
        ], 
        [
            "instructional designer", 
            _("instructional designer")
        ], 
        [
            "subject matter expert", 
            _("subject matter expert")
        ]
    ], 
    "cognitiveProcessValues": [
        [
            "analyse", 
            _("analyse")
        ], 
        [
            "implement", 
            _("implement")
        ], 
        [
            "collaborate", 
            _("collaborate")
        ], 
        [
            "compare", 
            _("compare")
        ], 
        [
            "share", 
            _("share")
        ], 
        [
            "compete", 
            _("compete")
        ], 
        [
            "understand", 
            _("understand")
        ], 
        [
            "prove", 
            _("prove")
        ], 
        [
            "communicate", 
            _("communicate")
        ], 
        [
            "contextualize", 
            _("contextualize")
        ], 
        [
            "control", 
            _("control")
        ], 
        [
            "cooperate", 
            _("cooperate")
        ], 
        [
            "create", 
            _("create")
        ], 
        [
            "decide", 
            _("decide")
        ], 
        [
            "define", 
            _("define")
        ], 
        [
            "describe", 
            _("describe")
        ], 
        [
            "discuss", 
            _("discuss")
        ], 
        [
            "design", 
            _("design")
        ], 
        [
            "self assessment", 
            _("self assessment")
        ], 
        [
            "explain", 
            _("explain")
        ], 
        [
            "extrapolate", 
            _("extrapolate")
        ], 
        [
            "innovate", 
            _("innovate")
        ], 
        [
            "investigate", 
            _("investigate")
        ], 
        [
            "judge", 
            _("judge")
        ], 
        [
            "motivate", 
            _("motivate")
        ], 
        [
            "observe", 
            _("observe")
        ], 
        [
            "organize", 
            _("organize")
        ], 
        [
            "organize oneself", 
            _("organize oneself")
        ], 
        [
            "plan", 
            _("plan")
        ], 
        [
            "practise", 
            _("practise")
        ], 
        [
            "produce", 
            _("produce")
        ], 
        [
            "recognize", 
            _("recognize")
        ], 
        [
            "remember", 
            _("remember")
        ], 
        [
            "write up", 
            _("write up")
        ], 
        [
            "consider", 
            _("consider")
        ], 
        [
            "connect", 
            _("connect")
        ], 
        [
            "represent", 
            _("represent")
        ], 
        [
            "solve", 
            _("solve")
        ], 
        [
            "simulate", 
            _("simulate")
        ], 
        [
            "summarize", 
            _("summarize")
        ], 
        [
            "value", 
            _("value")
        ]
    ], 
    "costValues": [
        [
            "yes", 
            _("yes")
        ], 
        [
            "no", 
            _("no")
        ]
    ], 
    "contextValues": [
        [
            "school", 
            _("school")
        ], 
        [
            "higher education", 
            _("higher education")
        ], 
        [
            "training", 
            _("training")
        ], 
        [
            "other", 
            _("other")
        ]
    ], 
    "typeValues": [
        [
            "operating system", 
            _("operating system")
        ], 
        [
            "browser", 
            _("browser")
        ]
    ], 
    "copyrightAndOtherRestrictionsValues": [
        [
            "yes", 
            _("yes")
        ], 
        [
            "no", 
            _("no")
        ]
    ], 
    "kindValues": [
        [
            "ispartof", 
            _("ispartof")
        ], 
        [
            "haspart", 
            _("haspart")
        ], 
        [
            "isversionof", 
            _("isversionof")
        ], 
        [
            "hasversion", 
            _("hasversion")
        ], 
        [
            "isformatof", 
            _("isformatof")
        ], 
        [
            "hasformat", 
            _("hasformat")
        ], 
        [
            "references", 
            _("references")
        ], 
        [
            "isreferencedby", 
            _("isreferencedby")
        ], 
        [
            "isbasedon", 
            _("isbasedon")
        ], 
        [
            "isbasisfor", 
            _("isbasisfor")
        ], 
        [
            "requires", 
            _("requires")
        ], 
        [
            "isrequiredby", 
            _("isrequiredby")
        ]
    ], 
    "semanticDensityValues": [
        [
            "very low", 
            _("very low")
        ], 
        [
            "low", 
            _("low")
        ], 
        [
            "medium", 
            _("medium")
        ], 
        [
            "high", 
            _("high")
        ], 
        [
            "very high", 
            _("very high")
        ]
    ], 
    "statusValues": [
        [
            "draft", 
            _("draft")
        ], 
        [
            "final", 
            _("final")
        ], 
        [
            "revised", 
            _("revised")
        ], 
        [
            "unavailable", 
            _("unavailable")
        ]
    ], 
    "sourceValues": [
        [
            "LOMv1.0", 
            _("LOMv1.0")
        ]
    ], 
    "structureValues": [
        [
            "atomic", 
            _("atomic")
        ], 
        [
            "collection", 
            _("collection")
        ], 
        [
            "networked", 
            _("networked")
        ], 
        [
            "hierarchical", 
            _("hierarchical")
        ], 
        [
            "linear", 
            _("linear")
        ]
    ], 
    "interactivityLevelValues": [
        [
            "very low", 
            _("very low")
        ], 
        [
            "low", 
            _("low")
        ], 
        [
            "medium", 
            _("medium")
        ], 
        [
            "high", 
            _("high")
        ], 
        [
            "very high", 
            _("very high")
        ]
    ], 
    "nameValues": [
        [
            "pc-dos", 
            _("pc-dos")
        ], 
        [
            "ms-windows", 
            _("ms-windows")
        ], 
        [
            "macos", 
            _("macos")
        ], 
        [
            "unix", 
            _("unix")
        ], 
        [
            "multi-os", 
            _("multi-os")
        ], 
        [
            "none", 
            _("none")
        ], 
        [
            "any", 
            _("any")
        ], 
        [
            "netscape communicator", 
            _("netscape communicator")
        ], 
        [
            "ms-internet explorer", 
            _("ms-internet explorer")
        ], 
        [
            "opera", 
            _("opera")
        ], 
        [
            "amaya", 
            _("amaya")
        ]
    ], 
    "intendedEndUserRoleValues": [
        [
            "teacher", 
            _("teacher")
        ], 
        [
            "author", 
            _("author")
        ], 
        [
            "learner", 
            _("learner")
        ], 
        [
            "manager", 
            _("manager")
        ]
    ], 
    "accessTypeValues": [
        [
            "universal", 
            _("universal")
        ], 
        [
            "non-universal", 
            _("non-universal")
        ]
    ], 
    "difficultyValues": [
        [
            "very easy", 
            _("very easy")
        ], 
        [
            "easy", 
            _("easy")
        ], 
        [
            "medium", 
            _("medium")
        ], 
        [
            "difficult", 
            _("difficult")
        ], 
        [
            "very difficult", 
            _("very difficult")
        ]
    ],
	"taxonpathSourceValues": [
	]	                      
};

lomesVocab = {
    "interactivityTypeValues": [
        [
            "active", 
            _("active")
        ], 
        [
            "expositive", 
            _("expositive")
        ], 
        [
            "mixed", 
            _("mixed")
        ]
    ], 
    "learningResourceTypeValues": [
        [
            "photograph", 
            _("photograph")
        ], 
        [
            "illustration", 
            _("illustration")
        ], 
        [
            "video", 
            _("video")
        ], 
        [
            "animation", 
            _("animation")
        ], 
        [
            "music", 
            _("music")
        ], 
        [
            "sound effect", 
            _("sound effect")
        ], 
        [
            "voice-over", 
            _("voice-over")
        ], 
        [
            "compound audio", 
            _("compound audio")
        ], 
        [
            "narrative text", 
            _("narrative text")
        ], 
        [
            "hypertext", 
            _("hypertext")
        ], 
        [
            "computer graphics", 
            _("computer graphics")
        ], 
        [
            "integrated media", 
            _("integrated media")
        ], 
        [
            "database", 
            _("database")
        ], 
        [
            "table", 
            _("table")
        ], 
        [
            "graph", 
            _("graph")
        ], 
        [
            "conceptual map", 
            _("conceptual map")
        ], 
        [
            "navigation map", 
            _("navigation map")
        ], 
        [
            "multimedia presentation", 
            _("multimedia presentation")
        ], 
        [
            "tutorial", 
            _("tutorial")
        ], 
        [
            "digital dictionary", 
            _("digital dictionary")
        ], 
        [
            "digital encyclopaedia", 
            _("digital encyclopaedia")
        ], 
        [
            "digital periodical publication", 
            _("digital periodical publication")
        ], 
        [
            "thematic or corporate webs/web portals", 
            _("thematic or corporate webs/web portals")
        ], 
        [
            "wiki", 
            _("wiki")
        ], 
        [
            "weblog", 
            _("weblog")
        ], 
        [
            "multimedia creation/edition tool", 
            _("multimedia creation/edition tool")
        ], 
        [
            "web design tool", 
            _("web design tool")
        ], 
        [
            "office tool", 
            _("office tool")
        ], 
        [
            "programming tool", 
            _("programming tool")
        ], 
        [
            "information/knowledgeanalysis/organization tool", 
            _("information/knowledgeanalysis/organization tool")
        ], 
        [
            "process/procedure supporting tools", 
            _("process/procedure supporting tools")
        ], 
        [
            "individual/cooperative/collaborative learning/working management tool", 
            _("individual/cooperative/collaborative learning/working management tool")
        ], 
        [
            "multimedia creation/edition service", 
            _("multimedia creation/edition service")
        ], 
        [
            "web design service", 
            _("web design service")
        ], 
        [
            "office service", 
            _("office service")
        ], 
        [
            "programming service", 
            _("programming service")
        ], 
        [
            "information/knowledgeanalysis/organization service", 
            _("information/knowledgeanalysis/organization service")
        ], 
        [
            "process/procedure supporting service", 
            _("process/procedure supporting service")
        ], 
        [
            "individual/cooperative/collaborative learning/working management service", 
            _("individual/cooperative/collaborative learning/working management service")
        ], 
        [
            "guided reading", 
            _("guided reading")
        ], 
        [
            "master class", 
            _("master class")
        ], 
        [
            "textual-image analysis", 
            _("textual-image analysis")
        ], 
        [
            "discussion activity", 
            _("discussion activity")
        ], 
        [
            "closed exercise or problem", 
            _("closed exercise or problem")
        ], 
        [
            "contextualized case problem", 
            _("contextualized case problem")
        ], 
        [
            "open problem", 
            _("open problem")
        ], 
        [
            "real or virtual learning environment", 
            _("real or virtual learning environment")
        ], 
        [
            "didactic game", 
            _("didactic game")
        ], 
        [
            "webquest", 
            _("webquest")
        ], 
        [
            "experiment", 
            _("experiment")
        ], 
        [
            "real project", 
            _("real project")
        ], 
        [
            "simulation", 
            _("simulation")
        ], 
        [
            "questionnaire", 
            _("questionnaire")
        ], 
        [
            "exam", 
            _("exam")
        ], 
        [
            "self assessment", 
            _("self assessment")
        ]
    ], 
    "aggregationLevelValues": [
        [
            "1", 
            "1"
        ], 
        [
            "2", 
            "2"
        ], 
        [
            "3", 
            "3"
        ], 
        [
            "4", 
            "4"
        ]
    ], 
    "purposeValues": [
        [
            "discipline", 
            _("discipline")
        ], 
        [
            "idea", 
            _("idea")
        ], 
        [
            "prerequisite", 
            _("prerequisite")
        ], 
        [
            "educational objective", 
            _("educational objective")
        ], 
        [
            "accessibility restrictions", 
            _("accessibility restrictions")
        ], 
        [
            "educational level", 
            _("educational level")
        ], 
        [
            "skill level", 
            _("skill level")
        ], 
        [
            "security level", 
            _("security level")
        ], 
        [
            "competency", 
            _("competency")
        ]
    ], 
    "roleMetaValues": [
        [
            "creator", 
            _("creator")
        ], 
        [
            "validator", 
            _("validator")
        ]
    ], 
    "roleValues": [
        [
            "author", 
            _("author")
        ], 
        [
            "publisher", 
            _("publisher")
        ], 
        [
            "initiator", 
            _("initiator")
        ], 
        [
            "terminator", 
            _("terminator")
        ], 
        [
            "validator", 
            _("validator")
        ], 
        [
            "editor", 
            _("editor")
        ], 
        [
            "graphical designer", 
            _("graphical designer")
        ], 
        [
            "technical implementer", 
            _("technical implementer")
        ], 
        [
            "content provider", 
            _("content provider")
        ], 
        [
            "technical validator", 
            _("technical validator")
        ], 
        [
            "educational validator", 
            _("educational validator")
        ], 
        [
            "script writer", 
            _("script writer")
        ], 
        [
            "instructional designer", 
            _("instructional designer")
        ], 
        [
            "subject matter expert", 
            _("subject matter expert")
        ]
    ], 
    "cognitiveProcessValues": [
        [
            "analyse", 
            _("analyse")
        ], 
        [
            "implement", 
            _("implement")
        ], 
        [
            "collaborate", 
            _("collaborate")
        ], 
        [
            "compare", 
            _("compare")
        ], 
        [
            "share", 
            _("share")
        ], 
        [
            "compete", 
            _("compete")
        ], 
        [
            "understand", 
            _("understand")
        ], 
        [
            "prove", 
            _("prove")
        ], 
        [
            "communicate", 
            _("communicate")
        ], 
        [
            "contextualize", 
            _("contextualize")
        ], 
        [
            "control", 
            _("control")
        ], 
        [
            "cooperate", 
            _("cooperate")
        ], 
        [
            "create", 
            _("create")
        ], 
        [
            "decide", 
            _("decide")
        ], 
        [
            "define", 
            _("define")
        ], 
        [
            "describe", 
            _("describe")
        ], 
        [
            "discuss", 
            _("discuss")
        ], 
        [
            "design", 
            _("design")
        ], 
        [
            "self assessment", 
            _("self assessment")
        ], 
        [
            "explain", 
            _("explain")
        ], 
        [
            "extrapolate", 
            _("extrapolate")
        ], 
        [
            "innovate", 
            _("innovate")
        ], 
        [
            "investigate", 
            _("investigate")
        ], 
        [
            "judge", 
            _("judge")
        ], 
        [
            "motivate", 
            _("motivate")
        ], 
        [
            "observe", 
            _("observe")
        ], 
        [
            "organize", 
            _("organize")
        ], 
        [
            "organize oneself", 
            _("organize oneself")
        ], 
        [
            "plan", 
            _("plan")
        ], 
        [
            "practise", 
            _("practise")
        ], 
        [
            "produce", 
            _("produce")
        ], 
        [
            "recognize", 
            _("recognize")
        ], 
        [
            "remember", 
            _("remember")
        ], 
        [
            "write up", 
            _("write up")
        ], 
        [
            "consider", 
            _("consider")
        ], 
        [
            "connect", 
            _("connect")
        ], 
        [
            "represent", 
            _("represent")
        ], 
        [
            "solve", 
            _("solve")
        ], 
        [
            "simulate", 
            _("simulate")
        ], 
        [
            "summarize", 
            _("summarize")
        ], 
        [
            "value", 
            _("value")
        ]
    ], 
    "costValues": [
        [
            "yes", 
            _("yes")
        ], 
        [
            "no", 
            _("no")
        ]
    ], 
    "contextValues": [
        [
            "classroom", 
            _("classroom")
        ], 
        [
            "laboratory", 
            _("laboratory")
        ], 
        [
            "real environment", 
            _("real environment")
        ], 
        [
            "home", 
            _("home")
        ], 
        [
            "mixed", 
            _("mixed")
        ], 
        [
            "teacher", 
            _("teacher")
        ], 
        [
            "family", 
            _("family")
        ], 
        [
            "tutor", 
            _("tutor")
        ], 
        [
            "schoolmate", 
            _("schoolmate")
        ], 
        [
            "independent", 
            _("independent")
        ], 
        [
            "blended", 
            _("blended")
        ], 
// 2014-04-07, JRF - this is obviously a mistake, that will have to be fixed eventually.
// "presencial" is not an English word, but the translation of 'face to face'.
// We don't fix it yet because it breaks compatibility with Agrega.
// Now we have 'face to face' twice,  and we lack a value for the variable Asistencia (Attendance).
// (not to be mistaken again with 'mixed environment' (variable Environment, Lugar) or
// Blended (a Modality)
        [
            "presencial", 
            _("presencial")
        ], 
        [
            "face to face", 
            _("face to face")
        ], 
        [
            "distance", 
            _("distance")
        ]
    ], 
    "typeValues": [
        [
            "operating system", 
            _("operating system")
        ], 
        [
            "browser", 
            _("browser")
        ]
    ], 
    "copyrightAndOtherRestrictionsValues": [
        [
            "propietary license", 
            _("propietary license")
        ], 
        [
            "free software license EUPL", 
            _("free software license EUPL")
        ], 
        [
            "free software license GPL", 
            _("free software license GPL")
        ], 
        [
            "dual free content license GPL and EUPL", 
            _("dual free content license GPL and EUPL")
        ], 
        [
            "other free software licenses", 
            _("other free software licenses")
        ], 
        [
            "public domain", 
            _("public domain")
        ], 
        [
            "not appropriate", 
            _("not appropriate")
        ], 
        [
            "intellectual property license", 
            _("intellectual property license")
        ], 
        [
            "creative commons: attribution", 
            _("creative commons: attribution")
        ], 
        [
            "creative commons: attribution - non derived work", 
            _("creative commons: attribution - non derived work")
        ], 
        [
            "creative commons: attribution - non derived work - non commercial", 
            _("creative commons: attribution - non derived work - non commercial")
        ], 
        [
            "creative commons: attribution - non commercial", 
            _("creative commons: attribution - non commercial")
        ], 
        [
            "creative commons: attribution - non commercial - share alike", 
            _("creative commons: attribution - non commercial - share alike")
        ], 
        [
            "creative commons: attribution - share alike", 
            _("creative commons: attribution - share alike")
        ], 
        [
            "license GFDL", 
            _("license GFDL")
        ]
    ], 
    "kindValues": [
        [
            "ispartof", 
            _("ispartof")
        ], 
        [
            "haspart", 
            _("haspart")
        ], 
        [
            "isversionof", 
            _("isversionof")
        ], 
        [
            "hasversion", 
            _("hasversion")
        ], 
        [
            "isformatof", 
            _("isformatof")
        ], 
        [
            "hasformat", 
            _("hasformat")
        ], 
        [
            "references", 
            _("references")
        ], 
        [
            "isreferencedby", 
            _("isreferencedby")
        ], 
        [
            "isbasedon", 
            _("isbasedon")
        ], 
        [
            "isbasisfor", 
            _("isbasisfor")
        ], 
        [
            "requires", 
            _("requires")
        ], 
        [
            "isrequiredby", 
            _("isrequiredby")
        ]
    ], 
    "semanticDensityValues": [
        [
            "very low", 
            _("very low")
        ], 
        [
            "low", 
            _("low")
        ], 
        [
            "medium", 
            _("medium")
        ], 
        [
            "high", 
            _("high")
        ], 
        [
            "very high", 
            _("very high")
        ]
    ], 
    "statusValues": [
        [
            "draft", 
            _("draft")
        ], 
        [
            "final", 
            _("final")
        ], 
        [
            "revised", 
            _("revised")
        ], 
        [
            "unavailable", 
            _("unavailable")
        ]
    ], 
    "sourceValues": [
        [
            "LOM-ESv1.0", 
            _("LOM-ESv1.0")
        ]
    ], 
    "structureValues": [
        [
            "atomic", 
            _("atomic")
        ], 
        [
            "collection", 
            _("collection")
        ], 
        [
            "networked", 
            _("networked")
        ], 
        [
            "hierarchical", 
            _("hierarchical")
        ], 
        [
            "linear", 
            _("linear")
        ]
    ], 
    "interactivityLevelValues": [
        [
            "very low", 
            _("very low")
        ], 
        [
            "low", 
            _("low")
        ], 
        [
            "medium", 
            _("medium")
        ], 
        [
            "high", 
            _("high")
        ], 
        [
            "very high", 
            _("very high")
        ]
    ], 
    "nameValues": [
        [
            "pc-dos", 
            _("pc-dos")
        ], 
        [
            "ms-windows", 
            _("ms-windows")
        ], 
        [
            "macos", 
            _("macos")
        ], 
        [
            "linux", 
            _("linux")
        ], 
        [
            "unix", 
            _("unix")
        ], 
        [
            "multi-os", 
            _("multi-os")
        ], 
        [
            "none", 
            _("none")
        ], 
        [
            "any", 
            _("any")
        ], 
        [
            "mozilla firefox", 
            _("mozilla firefox")
        ], 
        [
            "netscape communicator", 
            _("netscape communicator")
        ], 
        [
            "ms-internet explorer", 
            _("ms-internet explorer")
        ], 
        [
            "opera", 
            _("opera")
        ], 
        [
            "amaya", 
            _("amaya")
        ]
    ], 
    "intendedEndUserRoleValues": [
        [
            "learner", 
            _("learner")
        ], 
        [
            "special needs learner", 
            _("special needs learner")
        ], 
        [
            "gifted learner", 
            _("gifted learner")
        ], 
        [
            "learners late integration into the education system", 
            _("learners late integration into the education system")
        ], 
        [
            "learner with other specific educational support needs", 
            _("learner with other specific educational support needs")
        ], 
        [
            "general public", 
            _("general public")
        ], 
        [
            "individual", 
            _("individual")
        ], 
        [
            "group", 
            _("group")
        ], 
        [
            "teacher", 
            _("teacher")
        ], 
        [
            "tutor", 
            _("tutor")
        ], 
        [
            "family", 
            _("family")
        ], 
        [
            "information scientist", 
            _("information scientist")
        ], 
        [
            "computer scientist", 
            _("computer scientist")
        ], 
        [
            "manager", 
            _("manager")
        ], 
        [
            "education expert", 
            _("education expert")
        ], 
        [
            "subject matter expert", 
            _("subject matter expert")
        ]
    ], 
    "accessTypeValues": [
        [
            "universal", 
            _("universal")
        ], 
        [
            "non-universal", 
            _("non-universal")
        ]
    ], 
    "difficultyValues": [
        [
            "very easy", 
            _("very easy")
        ], 
        [
            "easy", 
            _("easy")
        ], 
        [
            "medium", 
            _("medium")
        ], 
        [
            "difficult", 
            _("difficult")
        ], 
        [
            "very difficult", 
            _("very difficult")
        ]
    ],
	"taxonpathSourceValues": Ext.create('Ext.data.Store', {
	    fields: ['id', 'text', 'purpose'],
	    data : [
	        { "id": "accesibilidad_LOM-ES_ca",
	          "text": "Accesibilidad LOM-ESv1.0 ca",
	          "purpose": "accessibility restrictions"
	        },
	        { "id": "accesibilidad_LOM-ES_en",
	          "text": "Accesibilidad LOM-ESv1.0 en",
	          "purpose": "accessibility restrictions"
	        },
	        { "id": "accesibilidad_LOM-ES_es",
	          "text": "Accesibilidad LOM-ESv1.0 es",
	          "purpose": "accessibility restrictions"
	        },
	        { "id": "accesibilidad_LOM-ES_eu",
	          "text": "Accesibilidad LOM-ESv1.0 eu",
	          "purpose": "accessibility restrictions"
	        },
	        { "id": "accesibilidad_LOM-ES_gl",
	          "text": "Accesibilidad LOM-ESv1.0 gl",
	          "purpose": "accessibility restrictions"
	        },
	        { "id": "accesibilidad_LOM-ES_va",
	          "text": "Accesibilidad LOM-ESv1.0 va",
	          "purpose": "accessibility restrictions"
	        },
	        { "id": "nivel_educativo_LOM-ES_ca",
	          "text": "Nivel educativo LOM-ESv1.0 ca",
	          "purpose": "educational level"
	        },
	        { "id": "nivel_educativo_LOM-ES_en",
	          "text": "Nivel educativo LOM-ESv1.0 en",
	          "purpose": "educational level"
	        },
	        { "id": "nivel_educativo_LOM-ES_es",
	          "text": "Nivel educativo LOM-ESv1.0 es",
	          "purpose": "educational level"
	        },
	        { "id": "nivel_educativo_LOM-ES_eu",
	          "text": "Nivel educativo LOM-ESv1.0 eu",
	          "purpose": "educational level"
	        },
	        { "id": "nivel_educativo_LOM-ES_gl",
	          "text": "Nivel educativo LOM-ESv1.0 gl",
	          "purpose": "educational level"
	        },
	        { "id": "nivel_educativo_LOM-ES_va",
	          "text": "Nivel educativo LOM-ESv1.0 va",
	          "purpose": "educational level"
	        },
	        { "id": "competencia_LOM-ES_ca",
	          "text": "Competencia LOM-ESv1.0 ca",
	          "purpose": "competency"
	        },
	        { "id": "competencia_LOM-ES_en",
	          "text": "Competencia LOM-ESv1.0 en",
	          "purpose": "competency"
	        },
	        { "id": "competencia_LOM-ES_es",
	          "text": "Competencia LOM-ESv1.0 es",
	          "purpose": "competency"
	        },
	        { "id": "competencia_LOM-ES_eu",
	          "text": "Competencia LOM-ESv1.0 eu",
	          "purpose": "competency"
	        },
	        { "id": "competencia_LOM-ES_gl",
	          "text": "Competencia LOM-ESv1.0 gl",
	          "purpose": "competency"
	        },
	        { "id": "competencia_LOM-ES_va",
	          "text": "Competencia LOM-ESv1.0 va",
	          "purpose": "competency"
	        },
	        { "id": "arbol_curricular_LOE_2006_ca",
	          "text": "Árbol curricular LOE 2006 ca",
	          "purpose": "discipline"
	        },
	        { "id": "arbol_curricular_LOE_2006_en",
	          "text": "Árbol curricular LOE 2006 en",
	          "purpose": "discipline"
	        },
	        { "id": "arbol_curricular_LOE_2006_es",
	          "text": "Árbol curricular LOE 2006 es",
	          "purpose": "discipline"
	        },
	        { "id": "arbol_curricular_LOE_2006_eu",
	          "text": "Árbol curricular LOE 2006 eu",
	          "purpose": "discipline"
	        },
	        { "id": "arbol_curricular_LOE_2006_gl",
	          "text": "Árbol curricular LOE 2006 gl",
	          "purpose": "discipline"
	        },
	        { "id": "arbol_curricular_LOE_2006_va",
	          "text": "Árbol curricular LOE 2006 va",
	          "purpose": "discipline"
	        },
	        { "id": "etb-lre_mec-ccaa_V.1.0_ca",
	          "text": "ETB-LRE MEC-CCAA V1.0 ca",
	          "purpose": "discipline"
	        },
	        { "id": "etb-lre_mec-ccaa_V.1.0_en",
	          "text": "ETB-LRE MEC-CCAA V1.0 en",
	          "purpose": "discipline"
	        },
	        { "id": "etb-lre_mec-ccaa_V.1.0_es",
	          "text": "ETB-LRE MEC-CCAA V1.0 es",
	          "purpose": "discipline"
	        },
	        { "id": "etb-lre_mec-ccaa_V.1.0_eu",
	          "text": "ETB-LRE MEC-CCAA V1.0 eu",
	          "purpose": "discipline"
	        },
	        { "id": "etb-lre_mec-ccaa_V.1.0_gl",
	          "text": "ETB-LRE MEC-CCAA V1.0 gl",
	          "purpose": "discipline"
	        },
	        { "id": "etb-lre_mec-ccaa_V.1.0_va",
	          "text": "ETB-LRE MEC-CCAA V1.0 va",
	          "purpose": "discipline"
	        }
	    ]
    })
};

