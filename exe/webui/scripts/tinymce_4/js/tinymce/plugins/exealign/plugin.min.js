 ...
 * Authors: José Miguel Andonegi (http://www.ulhi.hezkuntza.net/web/guest/inicio1) and Ignacio Gros (http://gros.es/)
 *
...
tinymce.PluginManager.add('exealign', function(editor, url) {
	...
	function checkBlock(command){
		...
		if (node.nodeName!="FIGURE" && node.nodeName!="IMG" && node.className.indexOf('mce-preview-object ')!=0) return false;
		
		var currK = "";
		var figure;
		
		if(node.className && node.className.indexOf("exe-figure")==0) {
			
				currK = node.className;
				figure = node;
				
		} else {
		
			var e = jQuery(node).parents();
			e.each(function(){
				if(this.className && this.className.indexOf("exe-figure")==0) {
					currK = jQuery(this).attr("class");
					figure = this;
				}
			});
