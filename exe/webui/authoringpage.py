...
class AuthoringPage(RenderableResource):
    ...
    def __getNodeOptions(self, node, depth):
        options = [(u'&nbsp;&nbsp;&nbsp;'*depth + node.titleLong)]
        for child in node.children:
            options += self.__getNodeOptions(child, depth+1)
        return options

    def select(self, object='', options=[], selection=None):
        """Adds a dropdown selection to a form"""
        html = u'<select '
        html += u'name="' + u'getTargetID' + '" '

        html += u'onchange="submitLink(\'' + u'getTargetID' + '\', \'' + object + '\');"'
